...
namespace MoreMountains.TopDownEngine
{
	...
	/// </summary>
	[AddComponentMenu("TopDown Engine/Weapons/SurviveCharacterAim")]
	public class SurviveCharacterAim : WeaponAim
	{
		protected Vector2 _inputMovement;
		protected bool _hasOrientation2D = false;
		protected bool _facingRightLastFrame;

		public SurvivePlayer Owner;
		private bool primaaryMovementDirectionFix = false;
        /// <summary>
        ...
        protected override void Initialization()
		{
			...
			//Owner = gameObject.MMGetComponentNoAlloc<Character>();
			//アクセスしやすく
			Owner.characterAim = this;
		...
		protected override void GetCurrentAim()
		{
			...
			if (((Owner.LinkedInputManager as SurviveInputManager).GetInputMode() == SurviveInputManager.InputMode.Gamepad) /*Owner.LinkedInputManager.SecondaryMovement != Vector2.zero*/)
			{
				_reticle.SetActive(false);
				AimControl = AimControls.SecondaryMovement;

			}
			else
			{
				if (Owner.LinkedInputManager.CrouchButton.State.CurrentState == MMInput.ButtonStates.ButtonDown)
				{
					Owner.CursolOn = false;
				}
				if (Owner.CursolOn)
				{
					if (Owner.LinkedInputManager.MousePosition != new Vector2(_mousePosition.x, _mousePosition.y))
					{
						_reticle.SetActive(true);
						AimControl = AimControls.Mouse;
					}
				}
				else
				{
					...
					if (Owner.LinkedInputManager.RunButton.State.CurrentState == MMInput.ButtonStates.ButtonPressed)
					{
						primaaryMovementDirectionFix = true;
					}
					else
					{
						primaaryMovementDirectionFix = false;
            ...
            switch (AimControl)
			{
				...
				case AimControls.PrimaryMovement:
					...
					if(primaaryMovementDirectionFix)
					{
						return;
					}
        ...
        {
            if (AimControl == AimControls.SecondaryMovement)
            {
                return;
            }
            if (GameManager.Instance.Paused)

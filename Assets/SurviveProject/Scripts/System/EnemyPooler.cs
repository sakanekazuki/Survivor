...
[AddComponentMenu("More Mountains/Tools/Object Pool/EnemyPooler")]
public class EnemyPooler : MMObjectPooler
{
	/// the list of objects to pool
	public List<PoolerEnemy> Pool;
	...
	public List<EnemyPooler> Owner { get; set; }
	...
	public override void FillObjectPool()
	{
		...
		switch (PoolingMethod)
		{
			...
			case MMPoolingMethods.OriginalOrderSequential:
				// we store our poolsizes in a temp array so it doesn't impact the inspector
				foreach (PoolerEnemy pooledGameObject in Pool)
				{
	...
	public override GameObject GetPooledGameObject()
	{
		...
		{
			case MMPoolingMethods.OriginalOrder:
				(pooledGameObject, id) = GetPooledGameObjectOriginalOrder();
				break;
			case MMPoolingMethods.RandomPoolSizeBased:
				(pooledGameObject, id) = GetPooledGameObjectPoolSizeBased();
				break;
			case MMPoolingMethods.RandomBetweenObjects:
				(pooledGameObject, id) = GetPooledGameObjectRandomBetweenObjects();
				break;
			case MMPoolingMethods.OriginalOrderSequential:
				(pooledGameObject, id) = GetPooledGameObjectOriginalOrderSequential();
				break;
			default:
	...
	/// </summary>
	/// <returns>The pooled game object.</returns>
	public  (GameObject, int) GetPooledGameObjectWithId()
	{
		GameObject pooledGameObject;
		int id = 0;
		switch (PoolingMethod)
		{
			case MMPoolingMethods.OriginalOrder:
				(pooledGameObject, id) = GetPooledGameObjectOriginalOrder();
				break;
			case MMPoolingMethods.RandomPoolSizeBased://ˆê’UŽg‚í‚È‚¢Œ`‚Å
				(pooledGameObject, id) = GetPooledGameObjectPoolSizeBased();
				break;
			case MMPoolingMethods.RandomBetweenObjects:
				(pooledGameObject, id) = GetPooledGameObjectRandomBetweenObjects();
		...
		return (pooledGameObject, id);
	}

	//public GameObject GetPooledGameObjectWithId(int id)
	//{
	//	GameObject pooledGameObject;

	//	pooledGameObject = GetPooledGameObjectOrderID(id);
	//	if (pooledGameObject != null)
	//	{
	//		_lastPooledObject = pooledGameObject;
	//	}
	...
	protected virtual (GameObject, int) GetPooledGameObjectOriginalOrder()
	{
		int newIndex;
		...
		PoolerEnemy searchedObject = GetPoolObject(Pool[_currentIndex].GameObjectToPool);

		if (_currentIndex >= _objectPool.PooledGameObjects.Count) { return (null, searchedObject.ID); }
		if (!searchedObject.Enabled) { _currentIndex++; return (null, searchedObject.ID); }
		...
		if (_objectPool.PooledGameObjects[_currentIndex].gameObject.activeInHierarchy)
		{
			...
			{
				_currentIndex++;
				return (findObject, searchedObject.ID);
			}
			...
			{
				_currentIndex++;
				return (AddOneObjectToThePool(searchedObject.GameObjectToPool), searchedObject.ID);
			}
			else
			{
				// if it can't expand we return nothing
				return (null, searchedObject.ID);
			}
		}
		else
		{
			...
			_currentIndex++;
			return (_objectPool.PooledGameObjects[newIndex], searchedObject.ID);
		}
	}
	//protected virtual GameObject GetPooledGameObjectOrderID(int id)
	//{
	//	int newIndex;
	//	// if we've reached the end of our list, we start again from the beginning
	//	if (_currentIndex >= Pool.Count)
	...
	/// </summary>
	/// <returns>The pooled game object original order.</returns>
	protected virtual (GameObject, int) GetPooledGameObjectOriginalOrderSequential()
	{
		// if we've reached the end of our list, we start again from the beginning
		if (_currentIndex >= Pool.Count)
		{
		...
		if (_currentIndex >= _objectPool.PooledGameObjects.Count) { return (null, searchedObject.ID); }
		if (!searchedObject.Enabled) { _currentIndex++; _currentCount = 0; return (null, searchedObject.ID); }
		...
		else
		{
			...
			_currentCount++;
			OrderSequentialResetCounter(searchedObject);
			return (_objectPool.PooledGameObjects[_currentIndex], searchedObject.ID);
		}
	}

	protected virtual void OrderSequentialResetCounter(PoolerEnemy searchedObject)
	{
	...
	protected virtual (GameObject, int) GetPooledGameObjectPoolSizeBased()
	{
		...
		//		PoolerEnemy searchedObject = GetPoolObject(_objectPool.PooledGameObjects[randomIndex].gameObject);
	...
	protected virtual (GameObject, int) GetPooledGameObjectRandomBetweenObjects()
	{
			   ...
			   && overflowCounter < _objectPool.PooledGameObjects.Count)
		{
			...
			{
				objectFound = true;
				return (newGameObject,Pool[randomIndex].ID);
			}
			else
			{
				...
				{
					return (AddOneObjectToThePool(Pool[randomIndex].GameObjectToPool), Pool[randomIndex].ID);
				}
			}
			randomIndex++;
			overflowCounter++;
	...
	protected virtual PoolerEnemy GetPoolObject(GameObject testedObject)
	{
		...
		int i = 0;
		foreach (PoolerEnemy poolerObject in Pool)
		{
	...
	{
		PoolerEnemy searchedObject = GetPoolObject(testedObject);
		if (searchedObject != null)
		{

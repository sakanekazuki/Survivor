...
public class SurviveFuma : SurviveProjectile
{
	/// <summary>
	...
	{
		if (SourceWeapon is SurviveWeaponShuriken)
		{
			var shuriken = SourceWeapon as SurviveWeaponShuriken;
			...
			if (data != null && data.exWeaponStatuses != null && data.exWeaponStatuses.Count > 0)
			{
				...
				{
					if (data.exWeaponStatuses[i].exWeaponStatus == AbilityData.ExWeaponStatusData.ExWeaponStatus.Embers)
					{
						var p = this.gameObject.MMGetComponentNoAlloc<SurviveWeaponProjectile>();
						p.WeaponInputStart();
					}

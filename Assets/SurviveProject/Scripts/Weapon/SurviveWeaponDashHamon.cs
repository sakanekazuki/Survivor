...
namespace MoreMountains.TopDownEngine
{
	...
	/// </summary>
	[AddComponentMenu("TopDown Engine/Weapons/SurviveWeaponDashHamon")]
	public class SurviveWeaponDashHamon : SurviveWeaponBase, MMEventListener<TopDownEngineEvent>, MMEventListener<DashEvent>, MMEventListener<KillEvent>
	{
		...
		public override void SetParameter(AbilityData.WeaponLevelData data, GameDefine.StatusAttribute attribute, int id)
		{
			...
			{
				//バーストではないパターンはこれ系
				(this as SurviveWeaponDashHamon).ProjectilesPerShot = currentData.Num + (Owner != null ? (Owner as SurvivePlayer).battleParameter.GetThrowNum(AbilityID) : 0);
			}
				...
				else
				{
					...
					else
					{
						...
						else // if we're in 2D
						{
							...
							//}wawaw
							var euler = Quaternion.Euler(0, 0, (float)((360 / currentData.Num) * projectileIndex));
							Vector3 newDirection = transform.right;
							projectile.SetDirection(euler * newDirection, euler * transform.rotation, true/*Owner.Orientation2D.IsFacingRight*/);
		...
		public virtual void DetermineSpawnPosition(int id)
		{
			if (Flipped)
			{
				if (FlipWeaponOnCharacterFlip)

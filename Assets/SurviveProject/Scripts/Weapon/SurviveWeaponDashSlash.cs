...
namespace MoreMountains.TopDownEngine
{
	...
	/// </summary>
	[AddComponentMenu("TopDown Engine/Weapons/SurviveWeaponDashSlash")]
	public class SurviveWeaponDashSlash : SurviveWeaponBase, MMEventListener<TopDownEngineEvent>, MMEventListener<DashEvent>, MMEventListener<KillEvent>
	{
		...
		public override void SetParameter(AbilityData.WeaponLevelData data, GameDefine.StatusAttribute attribute, int id)
		{
			...
			{
				//バーストではないパターンはこれ系
				(this as SurviveWeaponDashSlash).ProjectilesPerShot = currentData.Num + (Owner != null ? (Owner as SurvivePlayer).battleParameter.GetThrowNum(AbilityID) : 0);
			}
		...
		/// </summary>
		public virtual GameObject SpawnProjectile(Vector3 spawnPosition, int projectileIndex, int totalProjectiles, bool triggerObjectActivation = true, float angle = 0f)
		{
			...
			if (projectile != null)
			{
				...
				else
				{
					if (useDashDirection)
					{
						...
						var rotation = Quaternion.FromToRotation(Vector3.right, normD);
						if(angle != 0f)
                        {
							rotation *= Quaternion.Euler(0f,0f,angle);

						}
						projectile.SetDirection(dashDirection, rotation, true/*Owner.Orientation2D.IsFacingRight*/);
					}
					else
					{
		...
		public IEnumerator WeaponUseCoroutine()
		{
            ...
            for (int i = 0; i < ProjectilesPerShot; i++)
			{
				...
				if (currentData.exWeaponStatuses! != null && currentData.exWeaponStatuses.Exists(_ => _.exWeaponStatus == AbilityData.ExWeaponStatusData.ExWeaponStatus.DASH_SLASH_EX))
				{
					SpawnProjectile(SpawnPosition + dashDirection.normalized * Disatnce, i, ProjectilesPerShot, true, 90f);
					PlaySpawnFeedbacks();
		...
		public float killDamage = 0.03f;
		public float killDamageMax = 0.6f;
		public float killRange = 0.05f;
		public float killRangeMax = 1.0f;
		public int killCountAmount = 1000;

		public void OnMMEvent(KillEvent killEvent)
		{
			if (currentData.exWeaponStatuses != null && currentData.exWeaponStatuses.Exists(_ => _.exWeaponStatus == AbilityData.ExWeaponStatusData.ExWeaponStatus.KillPower))
			{
				killCount++;
			}
		}
		...
		protected override void SetBulletData(SurviveProjectile projectile)
		{
			...
			if (currentData.exWeaponStatuses != null && currentData.exWeaponStatuses.Exists(_ => _.exWeaponStatus == AbilityData.ExWeaponStatusData.ExWeaponStatus.KillPower))
			{
				//上書き
				var PermanentEffects =(Owner as SurvivePlayer).CalcPermanentDataStatus();
				float damage = (Owner as SurvivePlayer).battleParameter.ATTACK * ((currentData.DamageRate / 100f) + Mathf.Max(killDamageMax, (killCount / killCountAmount) * killDamage));
				...
				var size = currentData.Size * ((Owner as SurvivePlayer).battleParameter.GetSkillArea(main) + Mathf.Max(killRangeMax, (killCount / killCountAmount) * killRange));

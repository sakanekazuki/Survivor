...
namespace MoreMountains.TopDownEngine
{
	...
	/// </summary>
	[AddComponentMenu("TopDown Engine/Weapons/SurviveWeaponSubKirin")]
	public class SurviveWeaponSubKirin : SurviveWeaponBase, MMEventListener<TopDownEngineEvent>, MMEventListener<MainWeaponShotEvent>
	{
		...
		public override void SetParameter(AbilityData.WeaponLevelData data, GameDefine.StatusAttribute attribute, int id)
		{
			...
			{
				//バーストではないパターンはこれ系
				(this as SurviveWeaponSubKirin).ProjectilesPerShot = currentData.Num + (Owner != null ? (Owner as SurvivePlayer).battleParameter.GetThrowNum(AbilityID) : 0);
			}
		...
		public void WeaponUseStart(GameObject target)
		{
			mainShotCount++;
			if (mainShotCount >= currentData.Interval)
			{
				mainShotCount = 0;
				StartCoroutine(WeaponUse(target));
				if (WeaponBase != null)
				{
		...
		public IEnumerator WeaponUse(GameObject target)
		{
			...
			for (int i = 0; i < ProjectilesPerShot; i++)
			{
				...
				var currentTarget = DetectTarget();
				if (currentTarget != null)
                {
					SpawnPosition = currentTarget.transform.position;
				}
				SpawnProjectile(SpawnPosition, i, ProjectilesPerShot, true, target);
				PlaySpawnFeedbacks();
		...
		public void OnMMEvent(MainWeaponShotEvent mainShotEvent)
		{
			...
			else
			{
				WeaponBase.subDatas.Add(
					new SubData() { active = true, hitCallback = WeaponUseStart, weaponID = ID.ToString() }
					);
			}
		}
